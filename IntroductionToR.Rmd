...
author: "Smithsonian National Zoo & Conservation Biology Institute"
...
[R](https://cran.r-project.org/) is a very powerful statistical programming language that is used broadly by researchers around the world.  [R](https://cran.r-project.org/) is an attractive programming language because it is free, open source, and platform independent.  With all the libraries that are available (and those that are in rapid development), it is quickly becoming a one-stop shop for most of your analysis needs in ecological statistics.  Most academic statisticians now use [R](https://cran.r-project.org/), which has allowed for greater sharing of [R](https://cran.r-project.org/) code and analysis packages to implement a variety of recommended methodologies.  One of the very first things academics often ask when hiring someone is simply, "Can you describe your [R](https://cran.r-project.org/) or statistical programming experience?"  It is now critical to have this background to be competitive for scientific (and many other) positions.
...
[R](https://cran.r-project.org/) is available for Linux, MacOS X, and Windows (95 or later) platforms. Software can be downloaded from one of the [Comprehensive R Archive Network](https://cran.r-project.org/) (CRAN) mirror sites. It's best to choose the [R](https://cran.r-project.org/) mirror that is closest to your location.  Once installed, [R](https://cran.r-project.org/) will open a console where you run code. You can also work on a script file, where you can write and save your work. Other windows will show up on demand, such as the plot tab.

[RStudio](https://www.rstudio.com/products/rstudio/download/) is an enterprise-ready professional software tool that integrates with [R](https://cran.r-project.org/) and known as an integrated development environment (IDE).  It has some nice features beyond the normal [R](https://cran.r-project.org/) interface, which many users feel it is easier to use than [R](https://cran.r-project.org/) (Fig. 1). Once you have installed [R](https://cran.r-project.org/), you should also download and install [RStudio](https://www.rstudio.com/products/rstudio/download/).  For this course, we will work exclusively in [RStudio](https://www.rstudio.com/products/rstudio/download/).

![Figure 1. R-Studio Interface, including 4 panels: Source Editor, Console, Workspace Browser (and History), and Plots (and Files, Packages, and Help).](IntroToR_Fig1.png)

## Getting Help

One of the most useful commands in [R](https://cran.r-project.org/) is `?`.  All [R](https://cran.r-project.org/) functions should have an associated help file. At the command prompt (signified by `>` in your Console window), type `?` followed by any command and you will be prompted with a help tab for that command (e.g., `?mean` Fig. 2). You can also search through the help tab directly by searching functions on the search bar. 

![Figure 2. Getting help in R.](IntroToR_Fig2.png)

<br>
The internet also contains a vast quantity of useful information.  There are blogs, mailing lists, and various websites (e.g., https://stackoverflow.com/) dedicated to providing information about [R](https://cran.r-project.org/), its packages, and potential error messages that you may encounter (among other things).  The trick is usually determining the key terms to limit your search.  I generally start any web-based search with *"R-Cran"*, which limits and focuses the search.  Using "*R*" as part of your key terms does not, by itself, limit the search.

# Basic R Concepts

There are a few concepts that are important to keep in mind before you start coding. The fact that [R](https://cran.r-project.org/) is a programming language may deter some users who think “I can't program". This should not be the case for two reasons. First, [R](https://cran.r-project.org/) is an interpreted language, not a compiled one, meaning that all commands typed on the keyboard are directly executed without requiring you to build a complete program like in most computer languages (e.g., C, Pascal). Second, [R](https://cran.r-project.org/)'s syntax is very simple and intuitive. For instance, a linear regression can be done with the command `lm(y ~ x)` which means fitting a linear model with `y` as the response and `x` as a predictor.

In [R](https://cran.r-project.org/), in order for a command to be executed, a function always needs to be written with parentheses, even if there is nothing within the parentheses (e.g., `ls()` instead of `ls`). If you type the name of a function without parentheses, [R](https://cran.r-project.org/) will display the content of the function.  See what happens when typing `ls` in the Console.

When working with [R](https://cran.r-project.org/), variables, data, functions, results, are stored in the active memory of the computer in the form of objects that you assign with a name (`x = c(1,2,3)`). The user can then execute actions on these objects with operators (arithmetic, logical, comparison) and functions (e.g., `x + 3`).
...
It is also important to know that [R](https://cran.r-project.org/) discriminates between uppercase and lowercase letters in the names of objects, so that `x` and `X` can name two distinct objects.
...
If you prefer, you can also set your working directory using the point-and-click method: Session > Set Working Directory > Choose Directory ... as shown in Fig. 3.

![Figure 3. Setting working directory in RStudio](IntroToR_Fig3.png)

<br>
If you are using a pre-defined [RStudio](https://posit.co/download/rstudio-desktop/) instance, like we are here, there is no need to set your working directory.  This is very convenient, as the relative paths are maintained, allowing us to share share directories without a need to set the absolute path. 

Data are often stored in subfolders to maintain organization in your projects. In those cases, it is not necessary to provide the full path information when accessing the file, as long as the location of the file is within the root directory you have previously set or are working in. For instance, a table stored in a subfolder within your root directory named *Data* (`D:/Courses/NASA_EcologicalConservation/1_IntroductionToR`), can be accessed without providing the complete directory information. For example, you can point to the *Data* subfolder when reading the table as in the example below.  You can, however, always provide the full directory of a data file stored on your local drive to avoid confusion.
...
Since [R](https://cran.r-project.org/) is a programming language, we can store information as objects to avoid unnecessary repetition.  **Note**: Values are case sensitive (e.g., `x` is not the same as `X`)!
...
## Vectors
Vectors are a basic data structure in [R](https://cran.r-project.org/).  They contain a sequence of data and can contain characters, numbers, or be TRUE/FALSE values.  **Remember**: If you are unsure or need help, use the help function (e.g., `help(seq)` or `?seq`).  Below are several ways to create vectors in [R](https://cran.r-project.org/).
...
To see only part (i.e., a subset) of the data stored in a vector, you need to "ask" [R](https://cran.r-project.org/) to extract the information you want using square brackets (e.g., `[]`). Most commonly, you will indicate in square brackets the position of the data you want to extract (from beginning of the vector [1] to the Nth slot in the vector [n]).  
  ...
  >Show the first 5 values of a vector of 10 numbers from 1 to 10
...
```  
  ...
  >Show the 5th value of the same vector

```{r}
x[5]
```

  >Show the 2nd and 4th element of x
...
## Functions
[R](https://cran.r-project.org/) functions can be defined as a collection of arguments structured together for carrying out a definite task. Functions have optional input and output arguments that return a value. Custom functions can be easily constructed in [R](https://cran.r-project.org/). Most often, however, we will use built-in functions within base packages or other downloadable packages.
...
[R](https://cran.r-project.org/) also contains many pre-existing functions in the base software.  Numeric functions include `sum()`, `mean()`, `sd()`, `min()`, `max()`, `median()`, `range()`, `quantile()`, or `summary()`. Try a few of these on the numeric vectors you have created.
...
[R](https://cran.r-project.org/) can be used to perform basic calculations and report the results back to the user.
...
Relational operators (<,>, =, !=, >=, <=) evaluate a conditional expression and return TRUE or FALSE
...
# Installing SWIRL
SWIRL is a user-generated program (also called a **package** or **library**) for learning how to code in [R](https://cran.r-project.org/).  To access the tutorial information, you must first install the package to make it accessible.  In the Console window (bottom left), type the following and press ENTER:
