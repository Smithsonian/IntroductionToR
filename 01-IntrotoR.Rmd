...
[R](https://cran.r-project.org/) is a very powerful statistical programming language that is used broadly by researchers around the world.  [R](https://cran.r-project.org/) is an attractive programming language because it is free, open source, and platform independent.  With all the libraries that are available (and those that are in rapid development), it is quickly becoming a one-stop shop for all your analysis needs.  Most academic statisticians now use [R](https://cran.r-project.org/), which has allowed for greater sharing of [R](https://cran.r-project.org/) code or packages to implement their recommended methods.  One of the very first things academics often ask when hiring someone is simply, "Can you describe your [R](https://cran.r-project.org/) or statistical programming experience?"  It is now critical to have this background to be competitive for scientific (and many other) positions. 

Among the reasons to use [R](https://cran.r-project.org/) you have:  
...
##### Boolean Operators
And (&), Or (|), Not (!)
Notice that all non-zero numbers are considered TRUE
...
SWIRL is a user-generated program (also called a **package** or **library**) for learning how to code in **R**.  To access this, you must first install the package so it is accessible.  In the Console window (bottom left), type the following and press ENTER:
